//
// Created by ray on 02.02.2025.
//

// Read files and prints top k word by frequency (Multithreaded Version)
...
std::mutex counter_mutex; // Mutex для защиты доступа к общему счетчику
...
void count_words(std::istream& stream, Counter& counter) {
  ...
  // Обновляем общий счетчик с использованием мьютекса
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
	threads.emplace_back([=, &freq_dict] {
